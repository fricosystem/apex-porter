...
import {
  ...
  subscribeChaves,
  addChave as addChaveFS,
  setChave as setChaveFS,
  updateChave as updateChaveFS,
  removeChave as removeChaveFS,
...
function activateFeature(feature: string) {
  ...
  switch (feature) {
    ...
    case 'chaves':
      unsubs.push(
        subscribeChaves((data) => useAppStore.setState({ registrosChaves: data }))
      );
      break;
  }
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  login: async (email: string, password: string) => {
    ...
    try {
      ...
      // PORTEIRO sempre recebe as permissões padrão + permissão de Chaves (roll-out)
      let permissoes: PageType[];
      if (userCargo === 'PORTEIRO') {
        const base = userPermissoes.length === 0 ? PERMISSOES_PORTEIRO : userPermissoes;
        permissoes = base.includes('chaves') ? base : [...base, 'chaves'];
      } else {
  ...
  setAuthFromFirebase: (firebaseUser: FirebaseUser, firestoreData?: FirestoreUser | null) => {
    ...
    if (hasFullAccess) {
      permissoes = [
        ...
        'relatorios', 'cadastros', 'avisos', 'chaves', 'lista-negra', 'achados-perdidos',
      ...
      // Garante que o PORTEIRO sempre tenha a permissão de Controle de Chaves (roll-out)
      const base = firestoreData?.permissoes || PERMISSOES_PORTEIRO;
      permissoes = base.includes('chaves') ? base : [...base, 'chaves'];
    } else {
    ...
    updateUltimoLogin(firebaseUser.uid);
    // Persist permissão de Chaves adicionada automaticamente (roll-out para porteiros existentes)
    const storedPerm = firestoreData?.permissoes || [];
    if (cargo === 'PORTEIRO' && storedPerm.length > 0 && !storedPerm.includes('chaves')) {
      updateUserProfile(firebaseUser.uid, { permissoes }).catch(() => {});
    }
  ...
  registrarSaida: (id, detalhes?: string, ocorrencia?: string, pesoSaida?: number, porteiroSaida?: string, pesoApara?: { campo: 'pesoCarregado' | 'pesoVazio'; valor: number }, porteiroSaidaUid?: string, pessoasSaidaIds?: string[]) => {
    ...
    set((state) => ({
      registrosFluxo: state.registrosFluxo.map((r) => {
        ...
        if (pesoApara && r.categoria === 'pesagem_apara') {
          updated[pesoApara.campo] = pesoApara.valor;
        }
    ...
    if (pesoApara) {
      ...
      if (current?.categoria === 'pesagem_apara') {
        fsUpdate[pesoApara.campo] = pesoApara.valor;
      }
    }
    if (pesoSaida !== undefined && pesoSaida > 0) {
  ...
  // Controle de Chaves (Firestore-backed)
  registrosChaves: [],
  addRegistroChave: (registro) => {
    set((state) => ({ registrosChaves: [...state.registrosChaves, registro] }));
    const { id, ...data } = registro;
    setChaveFS(id, data).catch((err) => {
  ...
  registrarDevolucaoChave: (id, nomeDevolucao) => {
    ...
    const hours = now.getHours().toString().padStart(2, '0');
    const minutes = now.getMinutes().toString().padStart(2, '0');
    const horarioDevolucao = `${hours}:${minutes}`;
  ...
  removeRegistroChave: (id) => {
    set((state) => ({ registrosChaves: state.registrosChaves.filter((r) => r.id !== id) }));
    removeChaveFS(id).catch((err) => {
      console.warn('[Firestore] Falha ao remover registro de chave:', err);
    });
  },
}));

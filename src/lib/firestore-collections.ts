...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// PESAGEM DE APARA (Phase 3)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const PESAGEM_APARA_COL = 'pesagem_apara';

export function subscribePesagemApara(callback: (data: RegistroPesagemApara[]) => void): Unsubscribe {
  return subscribeCollection<RegistroPesagemApara>(PESAGEM_APARA_COL, callback);
}

export async function addPesagemApara(data: Omit<RegistroPesagemApara, 'id'>): Promise<string> {
  return addDocument(PESAGEM_APARA_COL, data as Record<string, unknown>);
}

export async function setPesagemApara(id: string, data: Omit<RegistroPesagemApara, 'id'>): Promise<void> {
  await setDocument(PESAGEM_APARA_COL, id, data as Record<string, unknown>);
}

export async function updatePesagemApara(id: string, data: Partial<RegistroPesagemApara>): Promise<void> {
  await updateDocument(PESAGEM_APARA_COL, id, data);
}
...
const CHAVES_COL = 'chaves';

export function subscribeChaves(callback: (data: RegistroChave[]) => void): Unsubscribe {
  return subscribeCollection<RegistroChave>(CHAVES_COL, callback);
}

export async function addChave(data: Omit<RegistroChave, 'id'>): Promise<string> {
  return addDocument(CHAVES_COL, data as Record<string, unknown>);
}

export async function setChave(id: string, data: Omit<RegistroChave, 'id'>): Promise<void> {
  await setDocument(CHAVES_COL, id, data as Record<string, unknown>);
}

export async function updateChave(id: string, data: Partial<RegistroChave>): Promise<void> {
  await updateDocument(CHAVES_COL, id, data);
}

export async function removeChave(id: string): Promise<void> {
  await deleteDocument(CHAVES_COL, id);
}

...
export default function FluxoPage() {
  ...
  // PESAGEM DE APARA — campo de peso que falta preencher (o oposto do informado no registro)
  const getPesoAparaFaltante = (r: RegistroFluxo): 'pesoCarregado' | 'pesoVazio' | null => {
    if (r.categoria !== 'pesagem_apara') return null;
    const carregado = (r as any).pesoCarregado ?? 0;
    const vazio = (r as any).pesoVazio ?? 0;
  ...
  const concluirSaida = (pessoasSaidaIds?: string[]) => {
    ...
    if (principalSelecionado && registroSaida.categoria === 'pesagem_apara') {
      ...
      if (campoFaltante) {
        ...
        if (valor <= 0) {
          toast.error(campoFaltante === 'pesoCarregado' ? 'Preencha o peso carregado' : 'Preencha o peso vazio');
          return;
        }
        pesoApara = { campo: campoFaltante, valor };
      }
    }
            ...
            <div className="space-y-5">
              ...
              {!selectedRegistro.horarioSaida && !selectedRegistro.inativo && (
                <>
                  ...
                  {/* Peso complementar — PESAGEM DE APARA: exibe apenas o campo que faltou no novo registro */}
                  {selectedRegistro.categoria === 'pesagem_apara' && (() => {
                      ...
                      <div className="space-y-3">
                        <div className="space-y-1.5">
                          ...
                          <Input
                            type="number"
                            placeholder="0"
                            value={pesoComplementarInput}
                            onChange={(e) => setPesoComplementarInput(e.target.value)}
                            className="text-base"
                          />
                        </div>
                      </div>
